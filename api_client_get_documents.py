...
documents_gateway_clшent = build_documents_gateway_http_client()
...
# Получение документа тарифа
get_tariff_document_response = documents_gateway_clшent.get_tariff_document(
    open_credit_card_account_response["account"]["id"]
)
print("Get tariff document response:", get_tariff_document_response, "\n")

# Получение документа контракта
get_contract_document_response = documents_gateway_clшent.get_contract_document(
    open_credit_card_account_response["account"]["id"]
)
